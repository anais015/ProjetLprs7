...
$date='';
//var_dump($_GET);
if (isset($_GET['ref_event'])){
    $evenement = new Evenement(array(
        'id'=> $_GET['ref_event']
    ));
//    var_dump($evenement->exist($bdd,$evenement->getId()));
    if ($evenement->exist($bdd,$evenement->getId())) {
        $detail=$evenement->detailEvenement($bdd);
//        var_dump($detail);
        $titre=$detail['nom_event'];
        $date=explode(" ",$detail['debut'])[0];
        $date=date_format(date_create($date),"d/m/Y");
        $debut=explode(" ",$detail['debut'])[1];
        $fin=explode(" ",$detail['fin'])[1];
        $debut=date_format(date_create($debut),"H:i");
        $fin=date_format(date_create($fin),"H:i");
    ...
    <div id="fh5co-register" style="background-image: url(../../style/images/job.jpg); height: 100%">
        ...
        <div class="row">
            <div class="col-md-offset-1 col-md-8 col-md-offset-2">
                <div class="text-left">
                    <h2><?=$titre;?></h2>
                    <p>Date : <?=$date;?> -  Horraires : <?=$debut." - ".$fin;?></p>
                    <p>Salle : <?=$salle;?></p>
